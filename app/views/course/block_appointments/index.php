...
<form <?= Request::isXhr() ? 'data-dialog="size=big"' : '' ?>
    ...
    <fieldset>
        ...
        <label for="block_appointments_end_day" class="col-3">
            <?= _('Enddatum') ?>
            <input type="text" class="size-s has-date-picker" data-date-picker='{">=":"#block_appointments_start_day"}'
                   id="block_appointments_end_day"
        ...
        <div id="block_appointments_days">
            ...
            <label for="block_appointments_days_0" class="col-2">
                <input
                    <?= empty($request['block_appointments_days']) || in_array('everyday', $request['block_appointments_days']) ? 'checked' : '' ?>
        ...
        <label for="block_appointments_termin_typ">
            ...
            <select clas="size-l" name="block_appointments_termin_typ" id="block_appointments_termin_typ">
                <? foreach ($GLOBALS['TERMIN_TYP'] as $key => $value) : ?>
                    <option
                            value="<?= $key ?>"
                            <?= $request['block_appointments_termin_typ'] == $key ? 'selected' : '' ?>>
    ...
    <? if (Config::get()->RESOURCES_ENABLE && ($selectable_rooms || $room_search)) : ?>
        ...
        <label for="block_appointments_date_count">
            ...
            <input type="text" name="block_appointments_date_count" id="block_appointments_date_count" class="size-s"
                   value="<?= $request['block_appointments_date_count'] ?: 1 ?>">
        </label>
