            ...
            <? if (count($temp_permissions)): ?>
                <? foreach ($temp_permissions as $permission): ?>
                    ...
                    <tr data-temp-perms="1" class="resource-temporary-permission-row">
                        <input type="hidden" name="permissions[permission_id][<?= htmlReady($permission->id) ?>]"
                        ...
                        <td>
                            ...
                            <input type="hidden" name="permissions[user_id][<?= htmlReady($permission->id) ?>]"
                                   value="<?= htmlReady($permission->user_id)?>">
                        </td>
                        <td data-sort-value="<?= htmlReady($permission_sort_key) ?>">
                            <select name="permissions[level][<?= htmlReady($permission->id) ?>]">
                                <option value="user"
                                        <?=
                        ...
                        <td class="DateTime">
                            <input type="text" class="has-date-picker"
                                   name="permissions[begin_date][<?= htmlReady($permission->id) ?>]"
                                   value="<?= date('d.m.Y', $permission->begin)?>">
                            <input type="text" class="has-time-picker"
                                   name="permissions[begin_time][<?= htmlReady($permission->id) ?>]"
                                   value="<?= date('H:i', $permission->begin)?>">
                        </td>
                        <td class="DateTime">
                            <input type="text" class="has-date-picker"
                                   name="permissions[end_date][<?= htmlReady($permission->id) ?>]"
                                   value="<?= date('d.m.Y', $permission->end)?>">
                            <input type="text" class="has-time-picker"
                                   name="permissions[end_time][<?= htmlReady($permission->id) ?>]"
