...
class StudyAreasLVGroupsCombinedWizardStep implements CourseWizardStep
{
    /**
    ...
    public function validate($values)
    {
        ...
        if ($category->module) {
            if (isset($values['LVGroupsWizardStep'])
                    ...
                    && !count($values[__CLASS__]['studyareas'])
                    && $values[__CLASS__]['step'] > $values['LVGroupsWizardStep']['step']) {
                $ok = false;
                $errors[] = _('Die Veranstaltung muss mindestens einem Studienbereich oder einer LV-Gruppe zugeordnet sein.');
            }
        } else {
            if (!$values[__CLASS__]['studyareas']) {
                $ok = false;
    ...
    public function storeValues($course, $values)
    {
        ...
        if ($areas_required
            || ($values[__CLASS__]['studyareas'] && is_array($values[__CLASS__]['studyareas'])))
        {
            $course->study_areas = SimpleORMapCollection::createFromArray(
                StudipStudyArea::findMany($values[__CLASS__]['studyareas'])
            );
            if ($course->store()) {
                return $course;
            } else {
