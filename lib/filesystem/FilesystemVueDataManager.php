...
class FilesystemVueDataManager
{
    ...
    public static function getFileVueData(FileType $file, FolderType $topFolder, $last_visitdate = null)
    {
        ...
        return [
            ...
            'size' => $file->getSize(),
            'author_url' => $file->getUser() && $file->getUserId() !== $GLOBALS['user']->id ? URLHelper::getLink('dispatch.php/profile?username=' . $file->getUser()->username) : "",
    ...
    public static function getFolderVueData(FolderType $folder, FolderType $topFolder)
    {
        ...
        $vue_folder = [
            ...
            'author_name' => $folder->user ? $folder->user->getFullname('no_title_rev') : '',
            'author_url' => $folder->user && $folder->user->id !== $GLOBALS['user']->id? URLHelper::getURL('dispatch.php/profile', ['username' => $folder->user->username]) : '',
